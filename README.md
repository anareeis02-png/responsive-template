...
README_MD = """# Template Responsivo — anareeis02-png

Template simples criado enquanto estudo GitHub e front-end. Mobile-first, com Grid/Flexbox e imagens responsivas.

Como usar localmente:
1. Rode `python3 create_site.py`
2. Abra http://localhost:8000 no navegador (o script tenta abrir automaticamente).

Como publicar no GitHub Pages:
1. Crie um repositório público.
2. Adicione a pasta `responsive-site` (ou só o index.html) e faça push para a branch `main`.
3. Em Settings → Pages, configure a source para a branch `main` e a pasta `/ (root)`.
4. Aguarde e acesse https://anareeis02-png.github.io/<repo-name>/
"""

def write_files(out_dir: Path):
    out_dir.mkdir(parents=True, exist_ok=True)
    (out_dir / "index.html").write_text(INDEX_HTML, encoding="utf-8")
    (out_dir / "README.md").write_text(README_MD, encoding="utf-8")
    print(f"Arquivos gerados em: {out_dir.resolve()}")

def serve_directory(out_dir: Path, port: int, do_open: bool):
    os.chdir(out_dir)
    Handler = http.server.SimpleHTTPRequestHandler
    with socketserver.TCPServer(("", port), Handler) as httpd:
        url = f"http://localhost:{port}/"
        print(f"Servidor rodando em {url}  (pressione Ctrl+C para parar)")
        if do_open:
            try:
                webbrowser.open(url)
            except Exception:
        ...
        try:
            ...
            print("\nServidor interrompido pelo usuário. Até mais!")

def parse_args():
    p = argparse.ArgumentParser(description="Gera site responsivo e inicia servidor local.")
    p.add_argument("--port", type=int, default=8000, help="Porta do servidor (padrão: 8000)")
    p.add_argument("--dir", type=str, default="responsive-site", help="Pasta de saída (padrão: responsive-site)")
    p.add_argument("--no-open", action="store_true", help="Não abre o navegador automaticamente")
    return p.parse_args()
...
def main():
    ...
    if out_dir.exists() and any(out_dir.iterdir()):
        print(f"Atenção: a pasta {out_dir} já existe e não está vazia.")
        ans = input("Deseja sobrescrever arquivos (s/n)? ").strip().lower()
        if ans not in ("s", "sim", "y", "yes"):
            print("Operação cancelada.")
            sys.exit(0)
    write_files(out_dir)
    serve_directory(out_dir, args.port, not args.no_open)
